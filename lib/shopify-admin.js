...
const shopifyThrottleUntil = new Map();
const sleep = (ms) => new Promise((resolve) => setTimeout(resolve, Math.max(0, ms)));

async function waitForShopifyGate(shop) {
  const delay = (shopifyThrottleUntil.get(shop) || 0) - Date.now();
  if (delay > 0) await sleep(delay);
}

function deferShopifyGate(shop, delayMs) {
  shopifyThrottleUntil.set(shop, Math.max(shopifyThrottleUntil.get(shop) || 0, Date.now() + Math.max(0, delayMs)));
}
...
function isGraphqlThrottled(errors) {
  return (Array.isArray(errors) ? errors : [errors]).some((error) =>
    String(error?.extensions?.code || "").toUpperCase() === "THROTTLED" || /throttl/i.test(String(error?.message || ""))
  );
}

function graphqlThrottleDelayMs(json, attempt, baseDelayMs) {
  const cost = json?.extensions?.cost || {};
  const status = cost.throttleStatus || {};
  const requested = Number(cost.requestedQueryCost ?? cost.actualQueryCost ?? 0);
  const available = Number(status.currentlyAvailable ?? 0);
  const restoreRate = Number(status.restoreRate ?? 0);
  if (restoreRate > 0 && requested > available) return Math.max(100, ((requested - available) / restoreRate) * 1000 + 100);
  return Math.min(20000, baseDelayMs * (2 ** attempt)) + Math.random() * Math.min(250, baseDelayMs);
}
...
class ShopifyAdmin {
  constructor(options = {}) {
    ...
    this.maxThrottleAttempts = Math.max(1, Math.min(8, Number(options.maxThrottleAttempts || process.env.SHOPIFY_THROTTLE_MAX_ATTEMPTS || 4)));
    this.throttleBaseDelayMs = Math.max(0, Number(options.throttleBaseDelayMs ?? process.env.SHOPIFY_THROTTLE_BASE_DELAY_MS ?? 1000));
  ...
  async graphql(query, variables = {}) {
    ...
    for (let attempt = 0; attempt < this.maxThrottleAttempts; attempt += 1) {
      await waitForShopifyGate(this.shop);
      ...
      const throttled = response.status === 429 || isGraphqlThrottled(json.errors || []);
      if (throttled && attempt + 1 < this.maxThrottleAttempts) {
        const delayMs = retryAfterMs(response) ?? graphqlThrottleDelayMs(json, attempt, this.throttleBaseDelayMs);
        deferShopifyGate(this.shop, delayMs);
        console.warn("[ShopifyAdmin] throttle retry", {
          ...this.graphqlLogContext(query, json.errors || []),
          kind: response.status === 429 ? "http_429" : "graphql_throttled",
          attempt: attempt + 1,
    ...
    throw new Error("Shopify GraphQL throttle retry exhausted.");
  }
  ...
  async product(id) {
    ...
    const data = await this.graphql(`query Product($id: ID!) {
      product(id: $id) {
        ...
        personalizer: metafield(namespace: "custom", key: "product_personalizer") { id value type compareDigest updatedAt }
        personalizerAddon: metafield(namespace: "custom", key: "product_personalizer_addon") { id value type compareDigest updatedAt }
      }
    }`, { id: gid });
  ...
  async ensurePersonalizerDefinitions(apply = false) {
    const definitions = [
      {
        name: "Product Personalizer",
        namespace: "custom",
        key: "product_personalizer",
        ownerType: "PRODUCT",
        type: "json",
        description: "Source-neutral storefront personalization form configuration.",
      },
      {
        name: "Product Personalizer Add-on",
        namespace: "custom",
        key: "product_personalizer_addon",
        ownerType: "PRODUCT",
        type: "json",
        description: "Technical marker for app-managed personalization fee variants.",
      },
    ];
    const results = [];
    for (const input of definitions) {
      const data = await this.graphql(`query PersonalizerDefinition($namespace: String!, $key: String!) {
      ...
      if (existing) {
        results.push({ action: "exists", definition: existing });
        continue;
      }
      if (!apply) {
        results.push({ action: "create", dryRun: true, input });
        continue;
      }
      const created = await this.graphql(`mutation CreatePersonalizerDefinition($definition: MetafieldDefinitionInput!) {
        metafieldDefinitionCreate(definition: $definition) {
          createdDefinition { id name namespace key }
          userErrors { field message }
        }
      }`, { definition: input });
      throwUserErrors(created.metafieldDefinitionCreate.userErrors);
      results.push({ action: "created", definition: created.metafieldDefinitionCreate.createdDefinition });
    }
    return results;
  }

  async setProductPersonalizer(productId, config, addonMarker = null, apply = false) {
    const product = await this.product(productId);
    const ownerId = product.id;
    const configValue = JSON.stringify(config);
    const markerValue = JSON.stringify(addonMarker || {});
    const inputs = [
      { ownerId, namespace: "custom", key: "product_personalizer", type: "json", value: configValue },
      { ownerId, namespace: "custom", key: "product_personalizer_addon", type: "json", value: markerValue },
    ];
    const changed = String(product.personalizer?.value || "") !== configValue || String(product.personalizerAddon?.value || "") !== markerValue;
    const result = {
      product: { id: product.id, title: product.title },
      changed,
      bytes: Buffer.byteLength(configValue),
      digest: crypto.createHash("sha256").update(configValue).digest("hex"),
    };
    if (!apply) return { ...result, dryRun: true, inputs: inputs.map((input) => ({ ...input, value: `<${Buffer.byteLength(input.value)} bytes>` })) };
    await this.ensurePersonalizerDefinitions(true);
    const data = await this.graphql(`mutation SetProductPersonalizer($metafields: [MetafieldsSetInput!]!) {
      metafieldsSet(metafields: $metafields) {
        metafields { id namespace key type updatedAt }
        userErrors { field message }
      }
    ...
    return { ...result, dryRun: false, metafields: data.metafieldsSet.metafields };
  }

  async findProductByHandle(handle) {
    const data = await this.graphql(`query ProductByHandle($query: String!) {
    ...
    }`, { query: `handle:${String(handle).replace(/[^a-z0-9-]/gi, "")}` });
    return data.products.nodes[0] || null;
  }

  async publishToOnlineStore(productId) {
    const publications = await this.graphql(`query PersonalizerPublications {
      publications(first: 50) { nodes { id name } }
    }`);
    const onlineStore = (publications.publications.nodes || []).find((item) => /online store/i.test(item.name || ""));
    if (!onlineStore) return { action: "skipped", warning: "Online Store publication was not found." };
    const data = await this.graphql(`mutation PublishPersonalizerAddon($id: ID!, $input: [PublicationInput!]!) {
    ...
    throwUserErrors(errors);
    return { action: "published", publicationId: onlineStore.id };
  }

  async ensurePersonalizationAddon(mainProductId, amounts = [], currencyCode = "USD", apply = false) {
    const numericId = String(mainProductId).split("/").pop();
    const normalizedAmounts = [...new Set((amounts || []).map(Number).filter((amount) => Number.isFinite(amount) && amount > 0).map((amount) => Math.round(amount * 100) / 100))].sort((a, b) => a - b);
    if (!normalizedAmounts.length) return { action: "none", marker: null, amounts: [] };
    const handle = `personalization-fee-${numericId}`;
    const existing = await this.findProductByHandle(handle);
    const values = normalizedAmounts.map((amount) => `$${amount.toFixed(2)}`);
    const input = {
      title: `Personalization fee ${numericId}`,
      handle,
      status: "ACTIVE",
      vendor: "Product Personalizer",
      tags: ["product-personalizer-addon"],
      productOptions: [{ name: "Fee", position: 1, values: values.map((name) => ({ name })) }],
      variants: normalizedAmounts.map((amount, index) => ({
        optionValues: [{ optionName: "Fee", name: values[index] }],
        price: amount.toFixed(2),
        taxable: false,
        inventoryItem: { tracked: false },
        sku: `PERSONALIZER-${numericId}-${Math.round(amount * 100)}`,
      })),
    };
    if (!apply) return { action: existing ? "update" : "create", dryRun: true, handle, input, amounts: normalizedAmounts };
    const data = await this.graphql(`mutation SyncPersonalizerAddon($identifier: ProductSetIdentifiers!, $input: ProductSetInput!) {
      productSet(identifier: $identifier, input: $input, synchronous: true) {
        product {
          id title handle
    ...
    }`, { identifier: existing ? { id: existing.id } : { handle }, input });
    throwUserErrors(data.productSet.userErrors);
    const product = data.productSet.product;
    const variants = {};
    for (const variant of product.variants.nodes || []) variants[Number(variant.price).toFixed(2)] = variant.id;
    await this.graphql(`mutation HidePersonalizerAddon($metafields: [MetafieldsSetInput!]!) {
      metafieldsSet(metafields: $metafields) { userErrors { field message } }
    }`, { metafields: [{ ownerId: product.id, namespace: "seo", key: "hidden", type: "number_integer", value: "1" }] }).then((result) => throwUserErrors(result.metafieldsSet.userErrors));
    const publication = await this.publishToOnlineStore(product.id);
    return {
      ...
      currencyCode,
      publication,
      marker: { schemaVersion: 1, productId: product.id, handle: product.handle, currencyCode, variants },
    };
  }
