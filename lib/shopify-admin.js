...
class ShopifyAdmin {
  ...
  async product(id) {
    ...
    const data = await this.graphql(`query Product($id: ID!) { product(id: $id) { id title handle status metafield(namespace: "custom", key: "amazon_customizer") { id value type compareDigest } } }`, { id: gid });
  ...
  async ensureCustomizerDefinition(apply = false) {
    ...
    const data = await this.graphql(`mutation CreateDefinition($definition: MetafieldDefinitionInput!) { metafieldDefinitionCreate(definition: $definition) { createdDefinition { id name namespace key } userErrors { field message code } } }`, { definition: input });
  ...
  async setCustomizer(productId, config, apply = false) {
    ...
    const data = await this.graphql(`mutation SetCustomizer($metafields: [MetafieldsSetInput!]!) { metafieldsSet(metafields: $metafields) { metafields { id namespace key type updatedAt } userErrors { field message code } } }`, { metafields: [input] });
    const payload = data.metafieldsSet;
    if (payload.userErrors.length) throw new Error(payload.userErrors.map((item) => item.message).join("; "));
    return { ...result, dryRun: false, metafield: payload.metafields[0] };
  }

  async ensureSurchargeProduct(amounts, apply = false) {
    const normalized = [...new Set((amounts || []).map(Number).filter((amount) => Number.isFinite(amount) && amount > 0))].sort((a, b) => a - b);
    const input = {
      title: "Customization Add-on", handle: "amazon-customization-addon", status: "ACTIVE",
    ...
    if (!normalized.length) return { action: "none", variants: {} };
    if (!apply) return { action: "upsert", dryRun: true, input, variants: Object.fromEntries(normalized.map((amount) => [amount, null])) };
    const data = await this.graphql(`mutation Surcharge($identifier: ProductSetIdentifiers, $input: ProductSetInput!) { productSet(identifier: $identifier, input: $input, synchronous: true) { product { id handle variants(first: 250) { nodes { id price selectedOptions { name value } } } } userErrors { field message code } } }`, { identifier: { handle: input.handle }, input });
    ...
    try {
      ...
      return { action: "upserted", productId: payload.product.id, variants, publicationWarning: error.message };
    }
    return { action: "upserted", productId: payload.product.id, variants, publication: publication && publication.name };
  }
  ...
  async uploadBuffer(buffer, { filename, mimeType = "application/octet-stream", alt = "", contentType = "FILE" } = {}, apply = false) {
    ...
    const staged = await this.graphql(`mutation Stage($input: [StagedUploadInput!]!) { stagedUploadsCreate(input: $input) { stagedTargets { url resourceUrl parameters { name value } } userErrors { field message code } } }`, {
  ...
  async createFileFromUrl(url, filename, alt, apply = false, contentType = "IMAGE") {
    ...
    const data = await this.graphql(`mutation FileCreate($files: [FileCreateInput!]!) { fileCreate(files: $files) { files { id fileStatus alt ... on MediaImage { image { url } } ... on GenericFile { url } } userErrors { field message code } } }`, { files: [input] });
  ...
  async waitForFile(id, attempts = 15) {
    for (let index = 0; index < attempts; index += 1) {
      ...
      if (data.node && data.node.fileStatus === "READY") return data.node;
  ...
  async cleanupOrderFiles({ olderThanDays = 30, apply = false } = {}) {
    ...
    for (let index = 0; index < candidates.length; index += 25) {
      ...
      const result = await this.graphql(`mutation DeleteFiles($ids: [ID!]!) { fileDelete(fileIds: $ids) { deletedFileIds userErrors { field message code } } }`, { ids });
