  ...
  function walk(node, parent = null, ancestors = [], path = []) {
    ...
    if (node.type === "OptionChooserComponent") {
      ...
      const options = (node.options || []).map((option) => ({
        ...
        outOfStock: Boolean(option.outOfStock || option.isOutOfStock || option.isSoldOut || option.unavailable || option.disabled || option.isDisabled || /out\s*of\s*stock|sold\s*out|unavailable/i.test(`${option.label || ""} ${option.name || ""}`)),
